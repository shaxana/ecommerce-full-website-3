...
function Users() {
  ...
  const { data, editId, uname, upassword } = useSelector((state) => state.users);
  ...
  const handleUpdate = (id) => {
    axios
      .put(`http://localhost:3000/users/${id}`, { name: uname, password: upassword })
      .then((res) => {
        dispatch(setUname(''));
        dispatch(setUpassword(''));
  ...
  return (
   ...
   style={{ margin:"0 auto"}}>
      <form style={{width:"40%",height:"40px",margin:"0 auto", padding:"20px"}}
       onSubmit={handleSubmit}>
        <input style={{width:"38%",height:"40px"}} 
         type="text" placeholder='Enter Name' value={name} onChange={e => setName(e.target.value)} />
        <input style={{width:"38%",height:"40px",marginLeft:"10px"}} 
         type="text" placeholder='Enter Password' value={password} onChange={e => setPassword(e.target.value)} />
        <button type="submit"
        style={{ background: "green", width:"60px",height:"40px",marginLeft:"10px", color:"white", border:"none" , borderRadius:"8px"}}
        >Add</button>
      </form>

 
      <table  style={{background:"white",marginLeft:"285px", marginTop:"60px", width:"1350px", height:"100vh", border:"1px solid black",}}>
        ...
        <tbody>
          {data && data.map((user) => (
            <tr style={{ position:"relative" ,left:"100px",margin:"55px"}}
             key={user.id}>
              <td>{user.id}</td>
              <td>
                {editId === user.id ? (
                  <input style={{padding:"15px",fontSize:"15px",textAlign:"center"}}
